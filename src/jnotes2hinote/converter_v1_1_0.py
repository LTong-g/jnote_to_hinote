#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Jnotes2Hinote v1.1.0 转换核心。

适用的逆向工程格式：
- Jideos 云记 3.2.3.2
- 华为笔记 15.0.14.295

这是自包含的 v1.1.0 转换核心。它直接在代码中生成经过验证的目标版本
所需的华为 PENCILENGINE 结构，不需要用户提供华为参考 `.hinote` 文件。

旧版基于参考文件的 v1.0.0 核心仍冻结在 converter_v1_0_0.py 中。
"""
...
def parse_jnotes(path: Path) -> JNote:
    """解析 v1.1.0 使用的 Jnotes 3.2.3.2 ZIP/Jzip 容器。"""
...
def _make_header_skeleton() -> bytes:
    """创建 v1.1.0 使用的最小华为笔记 15.0.14.295 PENCILENGINE 文件头。"""
    h = bytearray(196)
...
def convert(
    ...
    """将 `.Jnotes` 笔记转换为不依赖参考文件的华为 `.hinote` 压缩包。"""
